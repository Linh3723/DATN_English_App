...
class LessonVideoController {
    ...
    try {
      ...
      chewieController = ChewieController(
        ...
        errorBuilder: (context, errorMessage) {
          return Center(
            child: Text(
              'Lỗi: Không thể tải nội dung video',
              style: Theme.of(
                context,
  ...
  void markLessonAsCompleted(String courseId) async {
    if (courseId != null) {
      ...
      if (lessonIndex != -1) {
        ...
        final allLessonsCompleted = DummyDataService.isCourseUnlocked(courseId);
        ...
        if (isLastLesson && allLessonsCompleted) {
          ...
          Get.snackbar(
            'Hoàn thành bài học',
            'Bạn có thể chuyển tới bài học tiếp theo',
  ...
  void dispose(){
    videoPlayerController?.removeListener(() => onVideoProgressChanged(Get.parameters['courseId'] ?? ''));
